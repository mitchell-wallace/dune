...
func SessionMetaPath(dataDir string, sessionID int) string {
	return filepath.Join(contract.SessionDir(dataDir, sessionID), "meta.yaml")
}

func TranscriptPath(dataDir string, sessionID int) string {
	return filepath.Join(contract.SessionDir(dataDir, sessionID), "terminal.log")
}

func EnsureSessionDir(dataDir string, sessionID int) (string, error) {
	dir := contract.SessionDir(dataDir, sessionID)
...
func UpdateSessionMeta(dataDir string, sessionID int, update func(*SessionMeta) error) error {
	...
	meta := SessionMeta{Version: contract.SchemaVersion}
	...
	if meta.Version == 0 {
		meta.Version = contract.SchemaVersion
	}
...
func RebuildRepoProgress(dataDir, repoProgressPath string, activeBatch map[string]any) (RepoProgress, error) {
	...
	if len(sessions) > contract.RepoHistoryWindow {
		sessions = sessions[len(sessions)-contract.RepoHistoryWindow:]
	}
	...
	repo := RepoProgress{
		...
		HistoryWindow:  contract.RepoHistoryWindow,
