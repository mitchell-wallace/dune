...
- [x] 6.2 Smoke-verify the finalised commands against a sandbox built from the Dune sbx template (acceptance gates, design Migration §"Smoke verification"): `dune destroy --force` removes the instance (absent from `sbx ls --json`) and a subsequent `dune up` recreates it with persisted state intact; `dune doctor` does not change `sbx ls --json` for an absent/stopped sandbox and reports `pass` on a healthy host; `dune logs` surfaces a Dune-owned line plus `sbx policy log` records and exposes no `pipelock` subcommand; `dune ports` lists via `sbx ports <sandbox>`. Remove temporary sandboxes (`sbx rm`).
- [x] 6.3 Update docs for the finalised command surface, `dune logs` composition, diagnostics, and `dune doctor`.

Verification evidence (2026-06-19): `just smoke-sbx-commands --report tmp/sbx-commands-smoke.verify.20260619T010000Z.log` passed against `ghcr.io/mitchell-wallace/dune-sbx:0.4.8` with host `sbx v0.32.0`. The report records `sbx rm --force` and `sbx ports --publish/--unpublish` help output, absent- and stopped-sandbox `dune doctor --json` runs with unchanged `sbx ls --json`, host/sbx and sandbox checks passing, a profile persist sentinel surviving `dune destroy --force` plus recreate, `dune logs` output containing the host lifecycle log, `/var/log/dune/setup-persist.log`, and a blocked `docs.python.org:443` `sbx policy log` record, `dune logs pipelock` exiting unavailable, and `dune ports` listing via `sbx ports` plus the publish-time loopback/all-interfaces guidance. Cleanup removed `dune-sample-project-06-sbx-commands-smoke`; a final `sbx ls --json` was empty. `go build ./cmd/dune` and `go test ./...` passed after the smoke.
