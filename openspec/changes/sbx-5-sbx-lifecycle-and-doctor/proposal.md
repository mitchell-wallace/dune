...
The spikes sketched the shape. Spike 2's command mapping covers `dune up/down/rebuild/logs`, `dune destroy`/`rm`, and `dune ports` (with a loopback-vs-all-interfaces bind caveat for nested services). Spike 2 also found there is no single top-level `sbx logs` command, so Dune must define its own logs story. Spike 3 recommended keeping `dune logs` focused on `sbx` lifecycle/policy logs plus Dune-owned setup/runtime logs, with project app-service logs belonging to project-owned Compose. This change carries that direction and does not add in-template service health to doctor/logs.
...
- **NEW** Finalise the `dune` command mapping onto the sbx lifecycle, building on the `sbx-3` backend operations:
  ...
  - `dune ports` (**new first-class command**): wrap `sbx ports` list/publish (unpublish only if a port-removal spelling is confirmed via `sbx ports --help`; see D1), with guidance on the loopback-vs-all-interfaces bind caveat (services bound to sandbox loopback may not be publishable).
...
- **NEW** `dune doctor`: a read-only readiness command that does **not** start or enter the environment. It reports `sbx diagnose` (readiness/auth/daemon/version), Dune sbx template availability, sandbox status, profile persist readiness, and the egress policy baseline (from `sbx-4`). It deliberately excludes in-template service health so doctor stays focused on the core sbx backing. Output is concise and human-readable with an optional machine-readable (`--json`) mode.
...
- `sbx-lifecycle`: The finalised `dune` command surface over the sbx backend — `up`/`down`/`destroy`/`rebuild`/`logs`/`ports` — with `dune logs` composed from Dune-owned logs plus `sbx policy log`.
...
- `dune-doctor`: A read-only readiness command that inspects sbx/template/sandbox/profile-persist/policy readiness without starting the environment, with concise and optional JSON output.

## Impact

- **User-facing commands**: The `dune` verbs are finalised on sbx; `dune destroy` and `dune ports` are added; `dune logs` is recomposed (no `dune logs pipelock`).
...
- **Dependencies**: Requires `sbx-3` (backend ops and durable profile persistence) and `sbx-4` (egress baseline + `sbx policy log` access). Service-health checks are intentionally excluded, consistent with the `sbx-2` de-emphasis.

## Depends On

`sbx-3-sbx-runtime-backend` (command mapping and diagnostics target the sbx runtime, including durable profile persistence) and `sbx-4-sbx-network-and-secrets` (`dune logs` policy-log integration and the `dune doctor` policy-baseline check).
