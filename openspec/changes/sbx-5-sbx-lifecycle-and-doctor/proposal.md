...
- **NEW** Finalise the `dune` command mapping onto the sbx lifecycle, building on the `sbx-3` backend operations:
  ...
  - `dune ports` (**new first-class command**): wrap `sbx ports` list/publish/unpublish, with guidance on the loopback-vs-all-interfaces bind caveat (services bound to sandbox loopback may not be publishable).
