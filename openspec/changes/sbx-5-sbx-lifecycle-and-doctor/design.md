...
`dune logs` composes:
- Dune-owned setup/runtime logs (e.g. sandbox create/start/attach diagnostics and any Dune-managed in-sandbox setup output), from a documented location/command. The concrete source MUST be pinned during implementation, not left open: prefer the Dune-owned host-side log that the lifecycle commands already write (sandbox create/start/attach diagnostics emitted via the `sbx-3` runner seam) plus any in-sandbox Dune setup output at a path `sbx-2` records. The chosen source is recorded and asserted in tests (its presence in the composed output), so "what `dune logs` reads" is a verification point rather than a standing open question.
...
App-dependency service logs are intentionally **not** aggregated here; users read those via `docker compose logs` inside the sandbox for project-owned services. In-template service logs are not aggregated. There is no single `sbx logs` command to wrap (spike 2), so `dune logs` defines its own composition; the absence of `sbx logs` is itself a confirmed point, not an assumption.
