...
**Verification point (sbx binary — kit shape is unverified).** The spikes only read the kits docs (spike-3 §"Templates and kits direction"); no `sbx kit` command was ever exercised, and the YAML schema, the apply/reference verb, and how a sandbox is created *with* a kit are all unconfirmed against the installed `sbx`. Before this change documents a kit as the supported path, it MUST: (1) confirm the actual kit subcommand surface and YAML schema against the installed `sbx` (`sbx kit --help` / `sbx create --help` for any `--kit`-style flag) and record the verified spellings; (2) author one minimal mixin kit and confirm a sandbox created from the Dune template *with that kit* picks up its additions (e.g. an env var or a file the kit declares is present in the sandbox). Because kit application happens at sandbox-create time and cannot be observed by fakeRunner, this is a smoke-test gate against the `sbx` binary, not a unit assertion. If the installed `sbx` cannot apply kits as documented, Dune keeps customization in docs-only recipes and does not claim a kit build path it cannot verify.
