...
- [ ] 1.0 Kit surface against the installed binary — **surface recorded by spike 4** (`sbx kit add|inspect|pack|pull|push|validate`, all EXPERIMENTAL; `sbx create --kit <ref>` create-time apply; `commands.install` once-at-create, `commands.startup` per-boot, `environment.variables` reach non-login execs). Remaining: smoke-confirm against the **Dune** template specifically that a minimal mixin kit's addition is picked up (declared env var/file present in the sandbox), and confirm whether the schema carries network rules / a template-version field. If kits can't be applied as documented, fall back to docs-only recipes and do not claim a kit build path (D1).
