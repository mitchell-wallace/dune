...
- The concrete mechanism depends on what `sbx` supports. The spikes verified that workspace direct-mount persists across `stop`/`run` but did **not** test an additional persist mount or behavior across recreate. Options, in preference order: (a) mount a host-side profile-scoped persist dir (e.g. under `~/.local/share/dune/persist/<profile>`) into the sandbox at `/persist` if `sbx create` supports extra mounts; (b) an `sbx`-native per-profile volume if one exists; (c) if neither is available, accept that `rebuild` resets in-sandbox agent state and document it, keeping persistence per-sandbox until a mechanism lands. This is resolved during implementation (Open Questions).
...
- The durable-persist mechanism for profile-scoped state (extra host mount at `/persist` vs. an `sbx`-native per-profile volume vs. accepting that `rebuild` resets agent state), and how much cross-repo/profile persistence should survive — gated on what `sbx` supports for mounts/volumes (D3).
