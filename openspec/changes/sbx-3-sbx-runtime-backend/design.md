...
- **Mount-path delivery (resolved by spike 4): `sbx create` has no `--env` flag.** The verified options are (a) a **Dune-generated per-sandbox kit** passed via `--kit <dir>` whose `environment.variables` carries `DUNE_WORKSPACE=<path>` — spike 4 verified kit env vars reach even non-login `sbx exec <name> bash -c …` and survive restarts — or (b) in-sandbox derivation at login. Derivation must disambiguate between multiple rw virtiofs workspace mounts (the persist dir is also one), so the kit path is preferred where used; alternatively `sbx exec -e DUNE_WORKSPACE=<path> …` (verified flag) delivers it per-invocation for non-interactive commands. The caveat: `sbx kit` is marked **experimental**, so whichever delivery `sbx-2` D4 lands (kit env vs. derivation) is the contract `Ensure` consumes; the chosen create/exec argument construction is asserted in fakeRunner tests and the smoke test confirms `/workspace` resolves.
...
- **The mount-path delivery contract** — `sbx create` has no `--env` (spike 4); the contract is inherited from `sbx-2` D4's landed outcome: a Dune-generated per-sandbox kit's `environment.variables` (verified working, but `sbx kit` is experimental) vs. in-sandbox derivation (D4).
