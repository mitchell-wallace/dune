...
- **Carry forward the p10k TERM/COLORTERM forwarding.** The current Compose attach (`attachArgs`, `app.go`) forwards the caller's `TERM` and `COLORTERM` into the container with `-e` because `docker compose exec` otherwise defaults the container to `TERM=xterm` (8 colours), collapsing powerlevel10k's 256-colour prompt to monochrome. `sbx exec` has the same gap and supports `-e/--env`, so `Shell` must forward both the same way: `sbx exec -it -e TERM=<host TERM> -e COLORTERM=<host COLORTERM> -w <workspaceRoot> <instance> zsh`, emitting a `-e KEY=VALUE` pair only for each variable that is set in the host environment. Pin this argument construction (including the omit-when-unset behavior) in fakeRunner tests so the prompt-colour fix survives the cutover.
