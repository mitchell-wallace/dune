...
- Instance/sandbox naming and persistence: a Dune instance maps to a sandbox named `dune-<workspace-slug>-<profile>` (per workspace+profile), preserving today's mental model. Profile-scoped persisted state (agent credentials/config — today's `dune-persist-<profile>` volume contents exposed under `/persist`) is backed by a **durable, profile-scoped location decoupled from any single sandbox**, so it is shared across that profile's sandboxes and survives `rebuild` (which recreates the sandbox). The rest of the sandbox state persists via the sandbox lifecycle (until `rm`). The exact durable-persist mechanism depends on what `sbx` supports for mounts/volumes and is resolved in design.
...
- Replace the Docker prerequisite checks with sbx readiness validation, and replace `docker compose exec ... zsh` shell attach with `sbx exec -it -w <mounted repo path> <sandbox> zsh`.
