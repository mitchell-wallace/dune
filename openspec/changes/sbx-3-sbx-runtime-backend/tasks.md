...
- [ ] 4.1 Implement `Ensure` to create the sandbox from the Dune template with the workspace direct-mounted (verified shape: `sbx create --name <instance> --template <ref> shell <absolute host path>`), passing the profile-persist host dir as an extra positional workspace (design D3), then delivering the mount path via the sbx-2 D4 contract: `sbx exec -e DUNE_WORKSPACE=<absolute host path> <instance> bash -lc true` to fire the template hook that refreshes `/workspace` (design D4).
