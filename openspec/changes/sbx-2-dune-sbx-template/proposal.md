...
- It runs **s6-overlay** as PID 1 to supervise in-container Postgres/Redis/Mailpit plus the mandatory `setup-persist` oneshot, which collides with the Docker-enabled `sbx` template's own init that auto-starts `dockerd`.

Crucially, because the generic Docker-enabled template proves `docker compose` works **inside** the sandbox, the in-template Postgres/Redis/Mailpit services are no longer the parity-critical path they were under the Compose topology: a project owns its app dependencies via project-owned `docker compose` inside the sandbox. So rather than reconcile two init systems, this change **removes the in-container Postgres/Redis/Mailpit services and s6-overlay entirely** (spike-3 #4/#5), and re-homes the `setup-persist` credential-wiring oneshot onto a boot hook under the sbx template's native init.
...
- **NEW** A dedicated Dune sbx template image (the durable runtime artifact) that is Docker-enabled and reproduces Dune's batteries-included environment inside an `sbx` sandbox. It derives from, or replicates the important behavior of, Docker's Docker-enabled (`-docker`) shell sandbox template so Docker Engine and `docker compose` are available inside the sandbox. The sbx template's native Docker init owns PID 1; s6-overlay is dropped (see design D2a).
- **NEW** Remove the in-container Postgres/Redis/Mailpit services and s6-overlay from the template build; app dependencies come from project-owned `docker compose` inside the sandbox (spike-3 #4/#5). The `setup-persist` credential-wiring oneshot is re-homed onto a one-time boot hook (preferred: an `sbx`-native boot hook; fallback: an idempotent login-shell sentinel), and its output is written to a canonical in-container log path (`/var/log/dune/`) that `dune logs` (`sbx-5`) can read.
...
- `dune-sbx-template`: A dedicated, Docker-enabled Dune sbx template image that reproduces Dune's batteries-included environment inside an `sbx` sandbox, with in-container app services and s6-overlay removed (app deps via project-owned Compose), the `setup-persist` credential wiring re-homed onto a boot hook with a canonical in-container log path, deliberate `/workspace` compatibility, and a defined build/version/publish path.
...
- **Container build**: New/changed build inputs under `container/` (a new template Dockerfile / `container/sbx/` tree) to add Docker Engine + Compose inside the sandbox, drop s6-overlay and the app-service installs, and re-home `setup-persist`. Build time remains large (the spike notes a full base build can take ~12-15 minutes); contributors should validate with an ephemeral build.
- **Services**: The in-container Postgres/Redis/Mailpit services and the s6-overlay supervisor are **removed**. App dependencies come from project-owned `docker compose` inside the sandbox. Only the `setup-persist` credential-wiring step survives, re-homed onto a boot hook (a breaking change for projects that relied on auto-started in-container services).
...
- **Acceptance**: Centers on the spike-2 success criteria that actually gate the template, run against the installed `sbx` binary with the exact command forms the spikes proved — a sandbox starts from the Dune sbx template (`sbx create --name <name> --template <ref> shell <mount path>`); the attached shell resolves `/workspace` to the mounted repository; on a fresh sandbox the re-homed `setup-persist` boot hook has wired the agent-home symlinks into `/persist/agent` and logged to `/var/log/dune/setup-persist.log`, with `dockerd` responding and the sbx init not crash-looping; Docker Engine and `docker compose version` work; a nested `docker compose up` works (the canonical path for app dependencies); Playwright launches Chromium; `sbx ports <name> --publish <port>` can expose a nested service (bound to all sandbox interfaces) to the host; and `sbx policy deny network --sandbox <name> example.com` blocks both shell and nested-Docker traffic with useful `sbx policy log <name>` records. No in-container app services ship, so app-dependency readiness is validated only through the nested `docker compose` check.
