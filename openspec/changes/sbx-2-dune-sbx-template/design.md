...
The mount path is delivered to the template via the `DUNE_WORKSPACE` environment variable, which is the contract `sbx-3` D4 commits to. **Spike 4 resolved the mechanism options**: `sbx create` has **no `--env`/`-e` flag** (neither does `sbx run`); the verified delivery options are:
- **A Dune-generated per-sandbox kit** passed at create (`sbx create --kit <dir> …`) whose `environment.variables` carries `DUNE_WORKSPACE`. Spike 4 verified kit env vars reach even non-login `sbx exec <name> bash -c …` and survive stop/start. Caveat: `sbx kit` is experimental.
- **Per-invocation injection on `sbx exec`** (`-e DUNE_WORKSPACE=<path>` / `--env-file`, verified flags) for non-interactive backend commands, including the post-create `bash -lc` that fires the setup-persist hook.
- **In-sandbox derivation at login** (the repo is visible at its absolute host path). Workable, but ambiguous when multiple rw virtiofs workspaces are mounted (the profile-persist host dir is also a workspace, `sbx-3` D3), so derivation must filter for the primary workspace; a kit `initFile` with `${WORKDIR}` expansion is the native disambiguator if kits are used.

sbx-2 lands one of these as the verified contract recorded in 1.1/3.2 (recommended: exec-time `-e` for the hook invocation, with kit-based env as the create-time option if the experimental surface is accepted), and `sbx-3` D4 adopts whichever is recorded.
