...
**The sandbox-scoped baseline mechanism is resolved (spike 4): no local sandbox-scoped preset exists; Dune takes the verify-only path.** `sbx policy set-default <allow-all|balanced|deny-all>` is **global only** (no `--sandbox` flag), and `sbx policy profile` only lists profiles provided by *remote governance* policies (selected at create via `sbx create --profile`, not locally definable). Sandbox-scoped control remains per-domain `sbx policy allow/deny network --sandbox <name> <domain>`. Since Dune MUST NOT mutate global policy, the baseline behavior is the **verify-only** path below. (Two adjacent options exist without changing this conclusion: kits can declare `network.allowedDomains`/`deniedDomains` applied at create time — experimental, additive rules only, not a preset — and remote governance profiles can carry an org-defined baseline where present.)
...
The spikes verified `sbx policy log <instance> --limit <n>` records both `forward` (direct shell) and `transparent` (nested Docker) traffic, plus `forward-bypass` for allowed registry traffic. Spike 4 confirmed a structured form exists: `sbx policy log [SANDBOX] --json` (plus `--limit int`, `--type all|network`, `-q/--quiet`; the sandbox name is positional and optional). The wrapper may rely on the JSON form, with field names pinned in fakeRunner tests from observed output.
