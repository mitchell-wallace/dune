...
RUN arch="$(dpkg --print-architecture)" \
  && case "${arch}" in \
      amd64) s6_arch="x86_64"; mailpit_arch="amd64"; eza_arch="x86_64-unknown-linux-gnu"; delta_pkg_arch="amd64"; gitui_arch="x86_64" ;; \
      arm64) s6_arch="aarch64"; mailpit_arch="arm64"; eza_arch="aarch64-unknown-linux-gnu"; delta_pkg_arch="arm64"; gitui_arch="aarch64" ;; \
  ...
  && rm -rf /tmp/s6-overlay-noarch.tar.xz /tmp/s6-overlay-arch.tar.xz /tmp/mailpit.tar.gz /tmp/mailpit /tmp/eza.tar.gz /tmp/eza /tmp/git-delta.deb /tmp/gitui.tar.gz /tmp/gitui-install
