...
import (
	...
	"claudebox/internal/addons"
	"claudebox/internal/cli"
...
func run(argv []string) error {
	...
	default:
		return runSand(context.Background(), opts, paths)
	}
}
...
func runRebuild(ctx context.Context, opts cli.Options, paths repoPaths) error {
	...
	return runSand(ctx, cli.Options{Command: cli.CommandRun, WorkspaceInput: ref.Dir}, paths)
}

func runSand(ctx context.Context, opts cli.Options, paths repoPaths) error {
	ref, err := workspace.Resolve(defaultWorkspaceInput(opts.WorkspaceInput))
	if err != nil {
		return err
	}
	...
	buildAddonsArg := addons.BuildCSV(cfg.Addons, func(message string) {
		fmt.Fprintf(os.Stderr, "WARNING: %s\n", message)
	})
...
func applyConfiguredAddons(ctx context.Context, docker addonContainer, cfg domain.SandConfig, containerName, manifestPath string) error {
	...
	specs, err := addons.ParseManifest(manifestPath)
	if err != nil {
		return err
	}
	known := addons.IndexByName(specs)
